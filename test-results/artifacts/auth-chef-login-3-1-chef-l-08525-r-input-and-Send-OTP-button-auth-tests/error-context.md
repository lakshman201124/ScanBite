...
- Name: auth\chef-login.spec.ts >> 3.1 chef-login page shows phone number input and Send OTP button
- Location: tests\auth\chef-login.spec.ts:25:5
...
Locator: locator('button').filter({ hasText: /send otp|send code|otp/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /send otp|send code|otp/i }).first()
